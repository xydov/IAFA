param fichier := "archive_13/shift-scheduling.zplread" ;
do print fichier ;
...
subto daymax : forall<d,p> in Days*Personnes: (sum<s> in Services: assigned[p,d,s]) <= 1;
subto dayoff : forall<p,d,s> in Personnes*Days*Services: assigned[p,d,s] + dayOff[p, d] <= 1;
subto maxshift: forall<p,s> in Personnes*Services: (sum<d> in Days: assigned[p,d,s]) <= MaxShift[p,s];
subto intmax: forall <p> in Personnes:sum <d,s> in Days*Services: (assigned[p,d,s]*duree[s]) <= MaxTotalMinutes[p];
subto intmin: forall <p> in Personnes:sum <d,s> in Days*Services: (assigned[p,d,s]*duree[s]) >= MinTotalMinutes[p];

#3
subto maxcons: forall <p> in Personnes: forall <d> in {0..horizon-1-MaxConsecutiveShifts[p]}: (sum <dk> in {d..(d+MaxConsecutiveShifts[p])}: workingSched[p,dk]) <= MaxConsecutiveShifts[p];
subto forbidseq : forall<p,d,s1,s2> in Personnes*{0..horizon-2}*Services*Services: (ForbiddenSeq [s1,s2] + assigned [p,d,s1] + assigned [p,d+1,s2] ) <= 2;
